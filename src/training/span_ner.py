...
class SpanNERHead(nn.Module):
    ...
    def forward(
        ...
        for batch_idx in range(batch_size):
            ...
            for start_pos_idx in range(len(valid_positions)):
                start_idx = valid_positions[start_pos_idx].item()  # Convert to Python int
                
                # Maximum end position is either start + max_span or sequence end
                max_end_pos_idx = min(start_pos_idx + self.max_span_length, len(valid_positions))
                
                # Generate spans from start to all possible ends
                for end_pos_idx in range(start_pos_idx, max_end_pos_idx):
                    end_idx = valid_positions[end_pos_idx].item()  # Convert to Python int
                    
                    # Add candidate span (start, end) indices - both are Python ints
                    batch_spans.append((start_idx, end_idx))
        ...
        for batch_idx in range(batch_size):
            ...
            for start_idx, end_idx in candidate_spans[batch_idx]:
                ...
                # start_idx and end_idx are already Python ints, so no need for .item()
