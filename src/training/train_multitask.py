# src/training/train_multitask.py
import os
import torch
from peft import LoraConfig, get_peft_model
import ast
from transformers import AutoProcessor, BitsAndBytesConfig, Qwen2VLForConditionalGeneration, HfArgumentParser, Qwen2_5_VLForConditionalGeneration
from training.trainer import QwenTrainer
from training.multi_task_data import make_multi_task_data_module
from training.multi_task_model import MultiTaskQwen2VL
from training.params import DataArguments, ModelArguments, TrainingArguments
from training.train_utils import get_peft_state_maybe_zero_3, get_peft_state_non_lora_maybe_zero_3, safe_save_model_for_hf_trainer
import pathlib
from liger_kernel.transformers import apply_liger_kernel_to_qwen2_vl, apply_liger_kernel_to_qwen2_5_vl
from monkey_patch_forward import replace_qwen2_5_with_mixed_modality_forward, replace_qwen_2_with_mixed_modality_forward

local_rank = None

def rank0_print(*args):
    """Print only from the first rank in distributed training"""
    if local_rank == 0 or local_rank == '0' or local_rank is None:
        print(*args)

def find_target_linear_names(model, num_lora_modules=-1, lora_namespan_exclude=[], verbose=True):
    """Find target modules for LoRA adaptation"""
    linear_cls = torch.nn.modules.Linear
    embedding_cls = torch.nn.modules.Embedding
    lora_module_names = []

    for name, module in model.named_modules():
        if any(ex_keyword in name for ex_keyword in lora_namespan_exclude):
            continue
        if isinstance(module, (linear_cls, embedding_cls)):
            lora_module_names.append(name)
    
    if num_lora_modules > 0:
        lora_module_names = lora_module_names[-num_lora_modules:]
    if verbose:
        rank0_print(f"Found {len(lora_module_names)} lora modules: {lora_module_names}")
    return lora_module_names

def set_requires_grad(parameters, requires_grad):
    """Set requires_grad flag for parameters"""
    for p in parameters:
        p.requires_grad = requires_grad

def configure_vision_tower(model, training_args, compute_dtype, device):
    """Configure vision tower for training"""
    vision_tower = model.visual
    vision_tower.to(dtype=compute_dtype, device=device)

    vision_model_params = model.visual.parameters()
    set_requires_grad(vision_model_params, not training_args.freeze_vision_tower)
    
    # Handle merger specifically
    merger_params = model.visual.merger.parameters()
    set_requires_grad(merger_params, not training_args.freeze_merger)

def configure_llm(model, training_args):
    """Configure language model for training"""
    lm_head = model.lm_head.parameters()
    set_requires_grad(lm_head, not training_args.freeze_llm)

    llm_params = model.model.parameters()
    set_requires_grad(llm_params, not training_args.freeze_llm)


def train():
    """
    Main training function for multi-task learning with Qwen2-VL.
    
    This combines:
    1. Vision-language instruction-following (main task)
    2. Object detection with percentage-based coordinates (auxiliary task)
    3. Named entity recognition (auxiliary task)
    4. Optional contrastive learning between visual and textual features
    
    The percentage-based coordinate approach ensures seamless integration with
    the vision transformer architecture and provides resolution independence.
    """
    global local_rank
    
    # Parse arguments
    parser = HfArgumentParser((ModelArguments, DataArguments, TrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    
    # Setup model-specific optimizations
    use_liger = training_args.use_liger
    if "Qwen2.5" in model_args.model_id:
        # Apply monkey patches for mixed modality and optimizations
        replace_qwen2_5_with_mixed_modality_forward(use_liger=use_liger)
        if use_liger:
            apply_liger_kernel_to_qwen2_5_vl(fused_linear_cross_entropy=False)
    else:
        replace_qwen_2_with_mixed_modality_forward(use_liger=use_liger)
        if use_liger:
            apply_liger_kernel_to_qwen2_vl(fused_linear_cross_entropy=False)
    
    # Validate LoRA configuration
    if training_args.lora_enable and not training_args.freeze_llm:
        raise ValueError("If `lora_enable` is True, `freeze_llm` must also be True.")
    
    if not training_args.lora_enable:
        assert not training_args.vision_lora, \
            "Error: training_args.lora_enable is not enabled, but training_args.vision_lora is enabled."
        
    if training_args.vision_lora and not training_args.freeze_vision_tower:
        raise ValueError("If `vision_lora` is True, `freeze_vision_tower` must also be True.")
    
    # Setup LoRA configuration
    if training_args.lora_namespan_exclude is not None:
        training_args.lora_namespan_exclude = ast.literal_eval(training_args.lora_namespan_exclude)
    else:
        training_args.lora_namespan_exclude = []
    
    if not training_args.vision_lora:
        training_args.lora_namespan_exclude += ["visual"]
    
    # Set local rank for distributed training
    local_rank = training_args.local_rank
    
    # Determine computation dtype
    compute_dtype = (torch.float16 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32))
    
    # Configure quantization if needed
    bnb_model_from_pretrained_args = {}
    if training_args.bits in [4, 8]:
        print("Using quantization with bits:", training_args.bits)
        bnb_model_from_pretrained_args.update(dict(
            device_map={"": training_args.device},
            quantization_config=BitsAndBytesConfig(
                load_in_4bit=training_args.bits == 4,
                load_in_8bit=training_args.bits == 8,
                llm_int8_skip_modules=["visual"],
                llm_int8_threshold=6.0,
                llm_int8_has_fp16_weight=False,
                bnb_4bit_compute_dtype=compute_dtype,
                bnb_4bit_use_double_quant=training_args.double_quant,
                bnb_4bit_quant_type=training_args.quant_type,
            )
        ))
    
    print("BNB model from pretrained args:", bnb_model_from_pretrained_args)
    # Load base model
    rank0_print(f"Loading base model {model_args.model_id}...")
    base_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
        model_args.model_id,
        torch_dtype=compute_dtype,
        attn_implementation="flash_attention_2" if not training_args.disable_flash_attn2 else "sdpa",
        # device_map="auto",
        **bnb_model_from_pretrained_args
    )
    # print("Base model device:", base_model.device)
    
    # Disable caching during training
    base_model.config.use_cache = False
    # model_to_configure = model
    # Configure model components
    configure_llm(base_model, training_args)
    configure_vision_tower(base_model, training_args, compute_dtype, training_args.device)
    
    # Create multi-task model
    rank0_print("Creating multi-task model...")
    model = MultiTaskQwen2VL(
        base_model=base_model,
        object_detection_layer=data_args.object_detection_layer,
        ner_layer=data_args.ner_layer,
        vision_layer_percentage=data_args.vision_layer_percentage,  # Use 0.5 for 50%
        language_layer_percentage=data_args.language_layer_percentage,
        num_object_classes=data_args.num_object_classes,
        num_entity_types=data_args.num_entity_types,
        det_loss_weight=data_args.detection_loss_weight,
        ner_loss_weight=data_args.ner_loss_weight,
        contrastive_loss_weight=data_args.contrastive_loss_weight
    )

    # print("Model device:", model.device)
    
    # Setup quantization if needed
    if training_args.bits in [4, 8]:
        base_model.config.torch_dtype = (
            torch.float32 if training_args.fp16 else 
            (torch.bfloat16 if training_args.bf16 else torch.float32)
        )
        from peft import prepare_model_for_kbit_training
        model = prepare_model_for_kbit_training(
            model, 
            use_gradient_checkpointing=training_args.gradient_checkpointing,
            gradient_checkpointing_kwargs={"use_reentrant": True}
        )
    
    # # Setup gradient checkpointing
    # if training_args.gradient_checkpointing:
    #     base_model.enable_input_require_grads()
    #     training_args.gradient_checkpointing_kwargs = {"use_reentrant": True}
    # Setup gradient checkpointing
    if training_args.gradient_checkpointing:
        # This ensures the embeddings have requires_grad=True
        model.enable_input_require_grads()
        
        # # This enables gradient checkpointing in the base model
        # if hasattr(model.base_model, 'gradient_checkpointing_enable'):
        #     model.base_model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": True})
        
        # print("Gradient checkpointing enabled with input gradients")
        
        # Set the kwargs for the trainer
        training_args.gradient_checkpointing_kwargs = {"use_reentrant": True}
    
    # Configure LoRA if enabled
    if training_args.lora_enable:
        rank0_print("Setting up LoRA...")
        lora_namespan_exclude = training_args.lora_namespan_exclude
        peft_config = LoraConfig(
            r=training_args.lora_rank,
            lora_alpha=training_args.lora_alpha,
            target_modules=find_target_linear_names(
                base_model, 
                lora_namespan_exclude=lora_namespan_exclude,
                num_lora_modules=training_args.num_lora_modules
            ),
            lora_dropout=training_args.lora_dropout,
            bias=training_args.lora_bias
        )
        
        # Apply data type conversion if needed
        if training_args.bits == 16:
            if training_args.bf16:
                model.to(torch.bfloat16)
            if training_args.fp16:
                model.to(torch.float16)
        
        rank0_print("Adding LoRA to the base model...")
        model.base_model = get_peft_model(base_model, peft_config)
    
    # Adjust specific modules if using 4/8-bit training
    if training_args.bits in [4, 8]:
        from peft.tuners.lora import LoraLayer
        for name, module in model.named_modules():
            if isinstance(module, LoraLayer):
                if training_args.bf16:
                    module = module.to(torch.bfloat16)
            if 'norm' in name:
                module = module.to(torch.float32)
            
            if 'lm_head' in name or 'embed_token' in name:
                if hasattr(module, 'weight'):
                    if training_args.bf16 and module.weight.dtype == torch.float32:
                        module = module.to(torch.bfloat16)
    
    # Load tokenizer and processor
    processor = AutoProcessor.from_pretrained(model_args.model_id, padding_side="right")

    model.base_model.config.tokenizer_padding_side = processor.tokenizer.padding_side
    model.base_model.config.vision_lr = training_args.vision_lr
    
    # Create dataset and data loader
    rank0_print("Preparing datasets...")
    data_module = make_multi_task_data_module(
        model_id=model_args.model_id,
        processor=processor,
        data_args=data_args
    )
    
    # Create trainer instance
    rank0_print("Creating trainer...")
    trainer = QwenTrainer(
        model=model,
        processor=processor,
        args=training_args,
        **data_module
    )
    
    # Train model
    rank0_print("Starting training...")
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        trainer.train(resume_from_checkpoint=True)
    else:
        trainer.train()
    
    # Save trainer state
    trainer.save_state()
    
    # Enable caching for inference
    base_model.config.use_cache = True
    
    # Save model
    rank0_print("Saving model...")
    if training_args.lora_enable:
        # Save LoRA weights and configuration
        state_dict = get_peft_state_maybe_zero_3(
            model.base_model.named_parameters(), training_args.lora_bias
        )
        
        non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(
            model.base_model.named_parameters(), require_grad_only=False
        )
        
        if local_rank == 0 or local_rank == -1:
            model.base_model.config.save_pretrained(training_args.output_dir)
            model.base_model.save_pretrained(
                training_args.output_dir, state_dict=state_dict
            )
            torch.save(
                non_lora_state_dict, 
                os.path.join(training_args.output_dir, "non_lora_state_dict.bin")
            )
            
            # Save auxiliary task heads separately
            auxiliary_dict = {
                "detection_head": model.detection_head.state_dict(),
                "ner_head": model.ner_head.state_dict(),
                "config": {
                    "object_detection_layer": data_args.object_detection_layer,
                    "ner_layer": data_args.ner_layer,
                    "num_object_classes": data_args.num_object_classes,
                    "num_entity_types": data_args.num_entity_types,
                    "det_loss_weight": data_args.detection_loss_weight,
                    "ner_loss_weight": data_args.ner_loss_weight,
                    "contrastive_loss_weight": data_args.contrastive_loss_weight
                }
            }
            torch.save(
                auxiliary_dict, 
                os.path.join(training_args.output_dir, "auxiliary_heads.bin")
            )
    else:
        # Save full model
        safe_save_model_for_hf_trainer(trainer, output_dir=training_args.output_dir)
        
        # For full models, also save auxiliary heads configuration
        if local_rank == 0 or local_rank == -1:
            auxiliary_config = {
                "object_detection_layer": data_args.object_detection_layer,
                "ner_layer": data_args.ner_layer,
                "num_object_classes": data_args.num_object_classes,
                "num_entity_types": data_args.num_entity_types,
                "det_loss_weight": data_args.detection_loss_weight,
                "ner_loss_weight": data_args.ner_loss_weight,
                "contrastive_loss_weight": data_args.contrastive_loss_weight
            }
            
            # Save in JSON format for easier access
            import json
            with open(os.path.join(training_args.output_dir, "auxiliary_config.json"), 'w') as f:
                json.dump(auxiliary_config, f, indent=2)
    
    rank0_print("Training complete!")


if __name__ == "__main__":
    train()