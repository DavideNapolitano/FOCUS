...
class QwenTrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        """Save model with proper handling of weight-tied parameters"""
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if isinstance(self.accelerator.unwrap_model(self.model), supported_classes):
                ...
                # Check for weight tying and handle it
                if self.args.save_safetensors:
                    # Create a copy of the state dict to break weight sharing
                    clean_state_dict = {}
                    for k, v in state_dict.items():
                        # Create a clone for any potentially shared weights
                        clean_state_dict[k] = v.clone()
                    
                    try:
                        safetensors.torch.save_file(
                            clean_state_dict, os.path.join(output_dir, SAFE_WEIGHTS_NAME), 
                            metadata={"format": "pt"}
                        )
                    except RuntimeError as e:
                        # Fallback to PyTorch saving if safetensors still fails
                        logger.warning(f"Failed to save with safetensors: {e}")
                        torch.save(state_dict, os.path.join(output_dir, WEIGHTS_NAME))
                else:
                    torch.save(state_dict, os.path.join(output_dir, WEIGHTS_NAME))
        else:
            # Use the built-in save_pretrained which handles weight tying properly
